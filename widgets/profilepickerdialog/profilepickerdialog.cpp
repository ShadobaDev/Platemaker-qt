...
{
    ui->setupUi(this);
    // Two lines and chips, painted by the shared delegate in widgets/badge/. It reads each row from
    // the model, so the dialog no longer has to keep its rows alive for something else to index into.
    ui->listWidget->setItemDelegate(new BadgeItemDelegate(ui->listWidget));
...
void ProfilePickerDialog::setRows(const QList<Row> &rows, bool checkAllByDefault)
{
    ...
    // The delegate paints the title/summary/badges from the item's own data; the item text (plain
    // title) is what it paints on the first line and what feeds keyboard type-search.
    {
        ...
        for (const Row &r : rows) {
            ...
            item->setData(BadgeItemDelegate::k_badgesRole, QVariant::fromValue(r.titleBadges));
            item->setData(BadgeItemDelegate::k_subtitleRole, r.summary);
            item->setData(BadgeItemDelegate::k_subtitleBadgesRole,
                          QVariant::fromValue(r.summaryBadges));
