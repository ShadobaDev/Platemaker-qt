...
private:
    ...
    //! Rebuild the Workspace | Action split after either shell dock returns from floating. Qt re-docks
    //! the returning dock into the Left area but with a degenerate, overlapping geometry (the incumbent
    //! takes the whole area) that only a manual splitter drag fixed; removeDockWidget + splitDockWidget
    //! recreate the real split, then resizeDocks restores the pre-float proportion. Call deferred
    //! (singleShot) so it runs after Qt's own re-dock, not during it.
    void reestablishShellSplit();
    ...
    int m_workspaceDockWidthBeforeFloat = 0;    //!< Workspace dock width captured when either shell dock last floated; restored on re-dock (reestablishShellSplit).
    int m_actionDockWidthBeforeFloat    = 0;    //!< Action dock width captured at the same moment; feeds resizeDocks so the split proportion survives a float round-trip.
    ...
    QMenu *m_importCanvasMenu = nullptr;
    QMenu *m_importOutputMenu = nullptr;
    QMenu *m_exportCanvasMenu = nullptr;
