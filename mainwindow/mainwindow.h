...
private:
    ...
    void rebuildStatusAdvisories();   //!< Re-lays the status bar's permanent badges from the registry.
    ...
    static constexpr int k_statusBadgeSpacing     = 6;   //!< Gap between the status bar's advisory chips.
    ...
    /**
     * @brief The status bar's tray of chips — **one** widget holding them all, rebuilt whenever the
     *        registry moves.
     *
     * One rather than several because `QStatusBar` frames every item it is given, and a frame per chip
     * is a vertical rule between each pair. A single tray is a single frame, and the gap between chips
     * becomes its layout's spacing.
     */
    QWidget* m_statusBadges = nullptr;
